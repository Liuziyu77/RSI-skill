...
```text
请为当前正在运行的 Agent 安装这个仓库里的 RSI Skill。先执行对应平台的 dry-run，展示目标路径，确认不会覆盖已有 Skill 后再安装。
```

Agent 应根据当前平台选择 `codex`、`claude-code` 或 `openclaw`。这段请求已经明确授权安装；Agent 不需要让用户手工创建目录，但仍应先展示 dry-run 结果。
...
原始数据在 [`experiments/fixtures/retrieval_cases.json`](experiments/fixtures/retrieval_cases.json)，机器可读结果在 [`experiments/results/latest.json`](experiments/results/latest.json)，完整方法和后续配对任务评估方案见 [`experiments/README.md`](experiments/README.md)。
